...
// THUMB path (which is also where almost all retail DS game logic
// runs on the ARM9). Ported from gba-recomp; ARMv5TE differences from
// ARMv4T are confined to ARM-mode instructions, so the THUMB16 decode
// table is unchanged.
//
...
// bus does region routing the JIT doesn't model yet — and on the DS
// the bus has to consult VRAM banking, TCM mapping, etc.
...
export class Recompiler {
  ...
  jitInsns = 0;
  intInsns = 0;
  // Opt-in. Default OFF until the JIT has been baked in against retail
  // ROMs — for now the interpreter is the trusted path and the JIT is
  // exercised only by tests + explicit enable.
  enabled = false;

  // Shared linear memory for all compiled modules. 1 page = 64KB,
  // dwarfs what we use today (68 bytes) but leaves headroom for any
  ...
  // Host hooks bound at construction. Only bus I/O goes through them;
  // register and CPSR access happens entirely in WASM.
  private hooks: {
  ...
  // would make runFrame consume the whole frame budget in far fewer
  // iterations.
  tryDispatch(): number {
  ...
  private compile(startPc: number): CompiledBlock | null {
    ...
    // r16/w16 are declared for future format-10 (LDRH/STRH) support; tag
    // as used so unused-import lint stays quiet.
    void I_r16; void I_w16;
